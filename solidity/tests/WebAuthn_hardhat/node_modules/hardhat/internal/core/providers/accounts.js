...
class LocalAccountsProvider extends chainId_1.ProviderWrapperWithChainId {
    ...
    _initializePrivateKeys(localAccountsHexPrivateKeys) {
        const { bufferToHex, toBuffer, privateToAddress, } = require("@nomicfoundation/ethereumjs-util");
    ...
    async _getSignedTransaction(transactionRequest, chainId, privateKey) {
        const { AccessListEIP2930Transaction, Transaction } = await Promise.resolve().then(() => __importStar(require("@nomicfoundation/ethereumjs-tx")));
