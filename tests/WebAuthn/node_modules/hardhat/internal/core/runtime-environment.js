...
class Environment {
    ...
    constructor(config, hardhatArguments, tasks, extenders = [], experimentalHardhatNetworkMessageTraceHooks = [], userConfig = {}) {
         ...
         */
        this.run = async (name, taskArguments = {}, callerTaskProfile) => {
            ...
            try {
                return await this._runTaskDefinition(taskDefinition, resolvedTaskArguments, taskProfile);
            }
            catch (e) {
    ...
    async _runTaskDefinition(taskDefinition, taskArguments, taskProfile) {
        ...
        if (taskDefinition instanceof task_definitions_1.OverriddenTaskDefinition) {
            runSuperFunction = async (_taskArguments = taskArguments) => {
                ...
                try {
                    return await this._runTaskDefinition(taskDefinition.parentTaskDefinition, _taskArguments, parentTaskProfile);
                }
                finally {
        ...
        const uninjectFromGlobal = modifiedHreWithParentTaskProfile?.injectToGlobal() ??
            this.injectToGlobal();
        try {
