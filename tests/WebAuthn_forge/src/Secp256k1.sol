...
library Secp256k1 {
    ...
    function onCurve(uint[2] memory P) internal  returns (bool) {
        uint p = pp;
    ...
    function compress(uint[2] memory P) internal  returns (uint8 yBit, uint x) {
        x = P[0];
        yBit = P[1] & 1 == 1 ? 1 : 0;
    }

    /// @dev See Curve.decompress
    function decompress(uint8 yBit, uint x) internal  returns (uint[2] memory P) {
        uint p = pp;
    ...
    // outData: Rx, Ry, Rz
    function _add(uint[3] memory P, uint[3] memory Q) public  returns (uint[3] memory R) {
        if(P[2] == 0)
            return Q;
        if(Q[2] == 0)
            return P;
    ...
    // outData: Rx, Ry, Rz
    function _addMixed(uint[3] memory P, uint[2] memory Q) internal  returns (uint[3] memory R) {
        if(P[2] == 0)
            return [Q[0], Q[1], 1];
        if(Q[1] == 0)
            return P;
    ...
    function _double(uint[3] memory P) internal  returns (uint[3] memory Q) {
        uint p = pp;
        if (P[2] == 0)
            return P;
        uint Px = P[0];
    ...
    // Params: d, Px, Py
    // Output: Jacobian Q
    function _mul(uint d, uint[2] memory P) public  returns (uint[3] memory Q) {
        uint p = pp;
        if (d == 0) // TODO
