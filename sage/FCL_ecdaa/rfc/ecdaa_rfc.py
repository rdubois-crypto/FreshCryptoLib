...
def print_verbose(comment, a):
 if(_VERBOSE==true):
   print(comment, hex(a))
 return;  

def swap_word(int_a, bytelength):
  sums=0;
  cpt_i=0;
  while (cpt_i<bytelength):
  	sums=sums+( ((int_a>>8*cpt_i)&0xff)<<(8*(bytelength-cpt_i-1)));
  	cpt_i=cpt_i+1;
  return sums;
...
def BigNumberToB_BE(inVal, size):
 bin_a=(bin(inVal)[2:]);
...
def BigNumberToB_LE(inVal, size):
 le=swap_word(inVal, size)
 return BigNumberToB_BE(le,size);

 
def BigNumberToB(inVal, size):
 return BigNumberToB_BE(inVal,size);
...
#note that using compressed representation would spare gas
...
#Hash to integers (prior to modular  reduction by order)
def Hash_Bloc(int_a, size):
 ctx=H_Zp_init();
 ctx=H_Zp_update(ctx, int_a, size);
 return (int('0x'+ctx.hexdigest(),16));
...
def ECDAA_Verify(X,Y, Data, Data_s8, h_KRD, h_s8,  i_c,s,R,S,T,W,n):
 ...
 print("Input to Hash:", U,S,W,Data,h_KRD);
 
 print("Computed c2=",i_c2);
 ...
 print("Intermediate First pairing comparizon:e(R,Y)==e(S,P2)?","\n R:",R,"\n Y:",Y,"\n S:",S,"\n P2:",P2); 
