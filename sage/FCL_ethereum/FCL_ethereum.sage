...
def FCL_ecRecoverPoint(h,v,r,s,msg):
 Fq=GF(_256K1_ORDER);
 if r>_256K1_ORDER:
  return False;
 if s>_256K1_ORDER: 
...
def FCL_ecRecover(v,r,s,msg):
  msg=FCL_ecRecoverPoint
  e= int('0x'+keccak_256(msg).hexdigest(),16);
  
  return e>>12; #the 20 MSB bytes of keccak hash used as address
...
Q=FCL_ecRecover(0,28, r,s,msg);
print("recovery:",hex(pk[0]), hex(pk[1])); 	
